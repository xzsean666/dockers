...
export WALG_S3_ENDPOINT_SOURCE=${WALG_S3_ENDPOINT_SOURCE}
...
EOF
    ...
    sed -i 's/\${WALG_S3_PREFIX}/$WALG_S3_PREFIX/g' /etc/wal-g.d/env/vars
    sed -i 's/\${AWS_ACCESS_KEY_ID}/$AWS_ACCESS_KEY_ID/g' /etc/wal-g.d/env/vars
    sed -i 's/\${AWS_SECRET_ACCESS_KEY}/$AWS_SECRET_ACCESS_KEY/g' /etc/wal-g.d/env/vars
    sed -i 's/\${AWS_REGION}/$AWS_REGION/g' /etc/wal-g.d/env/vars
    sed -i 's/\${AWS_ENDPOINT_URL}/$AWS_ENDPOINT_URL/g' /etc/wal-g.d/env/vars
    sed -i 's/\${WALG_S3_ENDPOINT_SOURCE}/$WALG_S3_ENDPOINT_SOURCE/g' /etc/wal-g.d/env/vars
    sed -i 's/\${AWS_S3_FORCE_PATH_STYLE:-true}/$AWS_S3_FORCE_PATH_STYLE/g' /etc/wal-g.d/env/vars
    ...
    if curl -I --connect-timeout 10 --max-time 30 \"$AWS_ENDPOINT_URL\" 2>/dev/null | head -1 | grep -q \"HTTP\"; then
        ...
        echo '⚠️  R2端点网络连接测试失败，但这可能是正常的（R2可能不响应HEAD请求）'
    fi
...
-e "WALG_S3_ENDPOINT_SOURCE=$WALG_S3_ENDPOINT_SOURCE" \
