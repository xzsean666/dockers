...
ensure_dependencies() {
    ...
    if [ ! -f "node_modules/sqlite3/build/Release/node_sqlite3.node" ] && \
       ...
       ! find node_modules/sqlite3 -name "*.node" 2>/dev/null | grep -q .; then
        echo "检测到 sqlite3 原生模块未编译，执行 pnpm rebuild sqlite3..."
...
gsm_check_update() {
    ...
    if [ -n "$GITHUB_TOKEN" ]; then
        # 使用 store 方式持久化凭证，避免 credential helper 在 cron 环境中失效
...
if [ "$ENABLE_CICD" = "true" ] || [ "$ENABLE_CICD" = "1" ]; then
    ...
    # 导出所有环境变量到文件，供 cron 使用
    # 使用 declare -p 格式以正确处理包含特殊字符的变量
    {
        ...
        echo "export START_COMMAND='${START_COMMAND}'"
        [ -n "$POST_UPDATE_COMMAND" ] && echo "export POST_UPDATE_COMMAND='${POST_UPDATE_COMMAND}'"
        echo "export PID_FILE='/var/run/app.pid'"
    } > /etc/environment.cron
    
    # 创建 cron 任务文件
    # 每5分钟执行一次检查更新
    cat > /etc/cron.d/gsm-cron << 'EOFCRON'
SHELL=/bin/bash
# 每5分钟检查 Git 更新
*/5 * * * * root . /etc/environment.cron && cd /app && /bin/bash /docker-entrypoint.sh gsm >> /var/log/gsm-cron.log 2>&1
EOFCRON
